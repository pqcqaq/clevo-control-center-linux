...
- 性能：按原厂能力位过滤后的 DCHU 电源模式和风扇模式按钮
- 诊断：读取 DCHU 只读状态
...
“风扇”页中的自定义曲线当前只保存到 `settings.json`。开启后，总览/性能页的风扇模式行会额外显示 `曲线 1/2/3`，点击后保存当前本地选择并把对应 CPU/GPU 曲线作为应用意图；当前版本不写入 EC 风扇曲线表，也不把曲线数据作为裸 payload 暴露。
