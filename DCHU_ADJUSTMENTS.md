...
| 能力 | CLI | DCHU 路径 | 风险 |
|------|-----|-----------|------|
| 电源/性能档位 | `sudo clevo-keyboard-led dchu power-mode <0..3> --i-understand` | `0x79` / sub `0x19` | 会改 `EC.CPCM`、`DTTF`、平台性能状态并触发 SMI。参考 opencontrol，`0/1/2/3` 对应 `Quiet/Powersaving/Performance/Entertainment`。 |
| 风扇模式 | `sudo clevo-keyboard-led dchu fan-mode <mode> --i-understand` | `0x79` / sub `0x01` | 参考 opencontrol/opendchu，支持 `auto/max/silent/maxq/custom/turbo`，对应 `0/1/3/5/6/7`。会直接改 EC 风扇策略，测试时要观察温度和转速。 |
...
sudo clevo-keyboard-led dchu kbd-brightness 9 --i-understand
  -> integer 0x67
sudo clevo-keyboard-led dchu power-mode 0 --i-understand
  -> d7_fbuf: 0x80, rpm around 980/985
sudo clevo-keyboard-led dchu power-mode 1 --i-understand
  -> d7_fbuf: 0x02, rpm around 980/985
sudo clevo-keyboard-led dchu power-mode 2 --i-understand
  -> d7_fbuf: 0x08, rpm gradually dropped toward 750/750 in the sampled run
sudo clevo-keyboard-led dchu power-mode 3 --i-understand
  -> d7_fbuf: 0x02, rpm around 760-810 in the sampled run
clevo-keyboard-led dchu fan-mode auto
  -> blocked: dangerous write requires --i-understand
sudo clevo-keyboard-led dchu fan-mode auto --i-understand
