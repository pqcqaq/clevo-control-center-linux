...
### 阶段 12：灯光协议与实机修复
- [ ] 对照已逆向的 Clevo 原厂程序，确认键盘灯的命令、分区、亮度与特效语义
- [ ] 检查 Linux 用户态、内核模块与后台服务是否存在覆盖写入或协议不一致
- [ ] 在远端以可恢复方式验证基础区和必要的原厂命令
- [ ] 修复已确认的灯光行为问题，并完成本地与远端构建验证
- **状态：** in_progress

### 阶段 13：电池页面视觉优化
- [x] 重构信息层级，以充电窗口和策略状态作为主视觉
- [x] 合并预设、阈值和保护动作，移除卡片堆叠与冗余说明
- [x] 完成本地门禁与远端截图验证
- **状态：** complete

### 阶段 14：内部页面条件编译
- [x] 将诊断与高级导航、页面和领域状态限制为 debug 构建
- [x] 验证 debug 构建保留入口，release 构建完全移除入口
- [x] 完成本地与远端 release 验证和部署
- **状态：** complete

### 阶段 15：首次启动免责声明
- [x] 在缺少 `settings.json` 时阻止模块/服务初始化并显示产品内免责声明
- [x] 确认后原子写入默认配置，再加载模块并启动服务；退出时不落盘
- [x] 保持现有深灰/琥珀视觉并完成 debug/release、远端首次启动验证
- **状态：** complete

### 阶段 16：语言与主题个性化
- [x] 扩展设置模型，持久化语言模式和主题色，默认跟随系统语言与现有琥珀主题
- [x] 建立集中式语言偏好与主题调色板，接入导航、设置页和主要用户界面
- [x] 设置页提供紧凑的语言与主题选择，并支持即时生效
- [x] 完成迁移测试、debug/release 门禁与远端部署验证
- **状态：** complete

### 阶段 17：键盘区域能力与 60 FPS 灯效
- [x] 逆向确认 `KBTP/typeRule` 与单区、三区、逐键键盘的映射
- [x] 从硬件快照推导可控区域，并按能力裁剪设置页与灯光页
- [x] 后台服务实现不高于 16ms 的软件帧调度，动态灯效不依赖 EC 内建 effect
- [x] 实机测量单帧写入耗时、实际刷新频率和单区行为
- [x] 完成本地门禁、远端构建、部署与运行态验证
- **状态：** complete

### 阶段 18：自定义风扇曲线逆向诊断
- [ ] 对照原厂 `Read_WMI13`、`SetCustomFanTable` 与 `SetWMIPackage(14)` 完整调用链
- [ ] 审计 Linux 用户态序列化、内核 0x0E 包构造、模式切换和读回路径
- [ ] 读取远端当前曲线配置、模块状态与应用后的可观察状态，不盲写未知 EC 字段
- [ ] 确认“无效果”的根因、影响范围与安全修复方案
- **状态：** in_progress
...
| 灯光发现记录首次补丁匹配了错误标题 | 1 | 读取 `findings.md` 实际标题后按 `# 发现与决策` 插入，不重复错误上下文 |
| 首轮硬件灯效改造的 fmt check 报格式差异，并发现 1 个旧 `Mode` 导入 | 1 | 删除旧导入并运行 rustfmt；编译本身已通过 |
| 将蓝天 UI 的四个亮度采样值误判为硬件只有四档 | 1 | 按用户提供的硬件行为修正为连续百分比映射；保留四个原厂点作为映射测试，不再从 UI 限制推导固件限制 |
| 电池预设自绘按钮首轮保留了被覆盖的 `response` 变量 | 1 | 分配矩形时直接忽略初始 response，交互统一使用持久化 ID 的 response |
| 服务 PID 指向 zombie，旧存活检查仍返回 true | 1 | `process_is_running` 改读 `/proc/<pid>/stat`，将 `Z/X` 视为已退出，使现有 stale-lock 恢复逻辑能够执行 |
| GUI 先写子 PID 时，服务把自己误判为重复实例 | 1 | stale-lock 判断忽略当前进程自身 PID，同时继续拒绝其他存活 PID |
| `theme::apply` 仅为父模块可见，入口无法初始化主题 | 1 | 精确提升为 `pub(crate)` 并保持不导出到二进制外部 |
| 灯光页本地化补丁按旧语句顺序匹配失败 | 1 | 重新读取当前文件，拆成精确小段补丁，不重复整块匹配 |
| 首轮并行门禁被 `fan.rs` 一处 rustfmt 缩进差异中断 | 1 | 运行 rustfmt 后分别复跑 check、Clippy 和测试，避免非零汇总遮住结果 |
| 严格 Clippy 要求省略 `localized_fan_label` 的显式生命周期 | 1 | 按编译器建议使用生命周期省略，不添加 allow |
| Release Clippy 发现设置页拆出后遗留的 Debug-only imports | 1 | 对 `Button/Frame/page_header` 导入增加 `cfg(debug_assertions)`，保持 Release 零告警 |
| README 目录结构补丁首次按错误条目顺序匹配失败 | 1 | 读取实际行段后精确更新，不改动源码 |
| 远端测试命令未指定仓库目录，Cargo 在 HOME 找不到清单 | 1 | 后续统一使用绝对 `--manifest-path`，不依赖远端当前目录 |
| 远端 Cargo 测试无子进程空等并超过 SSH 184 秒超时 | 1 | 结束本轮卡住进程，改用 `--offline` 避免依赖索引轮询后复跑 |
| 远端 `--offline --all-features` 缺少可选 WGPU 依赖 `ash` | 1 | 不安装额外依赖，改验项目实际发布使用的默认 feature Release 配置 |
| 只用 `--manifest-path` 时 Cargo 未发现项目 `.cargo/config.toml`，查错缓存源 | 2 | 明确从远端仓库目录启动 Cargo，使 rsproxy source 配置与现有缓存生效 |
| Cargo 锁文件检索命令的 PowerShell 引号导致正则不完整 | 1 | 改用简单单引号模式 `rg 'ash|eframe|wgpu'` |
| 英文总览标题代码重叠且模式按钮仍按中文宽度绘制 | 1 | 中文维持原布局，英文标题代码后移、动作按钮从 88px 扩为 112px |
| 总览语言宽度计算首次补丁落到测试模块末尾 | 1 | 读取函数边界后把标题/动作宽度计算分别移回对应函数体 |
| 英文布局参数使 `overview_button_row` 达到 8 个参数 | 1 | 将语言与主题合并为页面外观上下文元组，不添加 Clippy allow 或新 helper 模块 |
| 远端未安装 `xdotool`，无法用坐标切换到设置页截图 | 1 | 保留已验证的最终总览实窗；改查可用 X11 输入工具，不重复相同命令 |
| PowerShell 变量直接拼接到 7-Zip `-o` 参数后未生成预期 appx 文件 | 1 | 先检查实际输出，再改用固定无歧义的 `-oC:\...` 参数分两步解包 |
